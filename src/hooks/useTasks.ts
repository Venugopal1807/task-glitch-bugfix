  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Deduplicate by id, remove malformed/undefined id rows
        const dedupedById = Array.from(
          new Map(finalData.filter(t => t.id).map(t => [t.id, t])).values()
        );

        if (isMounted) setTasks(dedupedById);
      } catch (e: any) {
